...
new Float:ChainDistance[5]={0.0,3000.0,4000.0,5000.0,9999.0};
new ChainDamage = 45;
    ...
    if(race==thisRaceID && pressed && IsPlayerAlive(client))
    {
        ...
        if(skill>0)
        {
            ...
            if(War3_SkillNotInCooldown(client,thisRaceID,ULT_LIGHTNING,true)&&!Silenced(client))
            {
                ...
                DoChain(client,distance,ChainDamage,true,0); // This function should also handle if there aren't targets
            }
        }
        else
        { 
